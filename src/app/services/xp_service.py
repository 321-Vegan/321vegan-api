...
def award_xp(
    ...
    action_type = db.query(XPActionType).filter(
        XPActionType.key == action_key,
        XPActionType.is_active.is_(True),
    ).first()

    if action_type is None:
        log.warning("Unknown or inactive XP action type: %s", action_key)
        return 0
    ...
    log.debug(
        "awarded %s xp to user %s for action %s (ref=%s/%s)",
        xp_awarded, user.id, action_key, reference_type, reference_id,
    )
    return xp_awarded
